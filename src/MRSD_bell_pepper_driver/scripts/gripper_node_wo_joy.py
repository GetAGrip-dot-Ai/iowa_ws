...
class MotorDriverROSWrapper:
    ...
    def __init__(self):
        ...
        # gripper close and open cb
        self.gripper_close_sub = rospy.Subscriber(
            'ag_gripper/gripper_close', Bool, self.cb_gripper_close)
        # close gripper msg
        self.close_msg = JointTrajectory()
        self.close_msg.joint_names = ['motor0', 'motor1', 'motor2', 'motor3']
        close_jtp = JointTrajectoryPoint()
        close_jtp.positions = self.q_zero + np.array([0.0, 3.0, 4.0, -3.5])
        ...
        self.gripper_cut_sub = rospy.Subscriber(
            'ag_gripper/gripper_cut', Bool, self.cb_gripper_cut)
        # cutting msg
